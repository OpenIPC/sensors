...
u32 vts_30fps; // 1125                        //for 29.091fps @ MCLK=36MHz
...
static int pCus_SwitchSensorID(ms_cus_sensor* handle, u32 SensorID)
{
    imx307_params* params = (imx307_params*)handle->private_data;
    ISensorIfAPI* sensor_if = handle->sensor_if_api;
    SENSOR_DMSG("[%s] ", __FUNCTION__);

    sensor_if->SetVIFSelectSource(SensorID);
    params->SensorID = SensorID;

    return SUCCESS;
}

static int pCus_GetCurSensorID(ms_cus_sensor* handle, u32* SensorID)
{
    imx307_params* params = (imx307_params*)handle->private_data;
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle)
{
    ...
    handle->pCus_sensor_SwitchgSensorId = pCus_SwitchSensorID;
    handle->pCus_sensor_GetCurSwtichSensorId = pCus_GetCurSensorID;

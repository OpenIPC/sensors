...
static int do_sensor_cfg(int SnrIdx, int BusType, int LaneNum, int MclkIdx, int Pdwn, int Rst) {
	...
	DrvVifSetMclk(SnrIdx, BusType, (MclkIdx == 0) ? 0 : 1, MclkIdx);
	DrvVifSensorPdwn(SnrIdx, BusType, (Pdwn == 1) ? E_VIF_CLK_POL_POS : E_VIF_CLK_POL_NEG);
	DrvVifSensorReset(SnrIdx, BusType, (Rst == 1) ? E_VIF_CLK_POL_POS : E_VIF_CLK_POL_NEG);
...
static ssize_t srcfg_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos) {
	...
	if (help) {
		printk("SensorId BusType MclkIdx PwdPin RstPin\n"
			...
			"example for imx415:\n"
			" -echo \"srcfg 0 1 4 0 1 1\" > /dev/srcfg\n");
	} else {

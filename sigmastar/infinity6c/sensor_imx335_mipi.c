...
////////////////////////////////////
//MIPI config begin.
#define SENSOR_MIPI_LANE_NUM        (lane_num) /*(lane_num)*/          //IMX335 Linear mode supports MIPI 2/4 Lane
#define SENSOR_MIPI_LANE_NUM_DOL    (lane_num) //(hdr_lane_num)//IMX335 DOL mode supports MIPI 4 Lane
...
static struct {     // LINEAR
    ...
    {LINEAR_RES_2, {2592, 1944, 3, 40}, {0, 0, 2592, 1944}, {"2592x1944@40fps"}}, // Modify it
    {LINEAR_RES_3, {2592, 1944, 3, 59}, {0, 0, 2592, 1944}, {"2592x1944@59fps"}}, // Modify it
...
static u32 imx335_get_linear_bayer_id(u32 res_idx)
{
    switch (res_idx) {
        case LINEAR_RES_4:
        case LINEAR_RES_5:
...
const static I2C_ARRAY Sensor_init_table_2lane_5m30fps[] =
{
    ...
    {0x3031,0x10},  //VMAX
    {0x3032,0x00},  //VMAX
    {0x3034,0xB8},  //HMAX   0x0258=>30fps   //2D0h: 720  //226h: 550   
    {0x3035,0x01},  //HMAX
    {0x3050,0x00},  //ADBIT
    {0x314C,0xB0},  //INCKSEL1
    {0x315A,0x02},  //INCKSEL2
...
/* "IMX335LQN All-pixel scan CSI-2_4lane 1188Mbps AD:10bit Output:10bit 27MHz Master Mode 30fps" */
    ...
    {0x3031,0x10},  //VMAX
    {0x3032,0x00},  //VMAX
    {0x3034,0x58},  //HMAX  Stock:0x0258   //2D0h: 720  //226h: 550
    {0x3035,0x02},  //HMAX 0x0208 35 fps
    {0x314C,0xB0},
    {0x315A,0x02},
    {0x3168,0x8F},
    {0x316A,0x7E},
...
const static I2C_ARRAY Sensor_init_table_4lane_5m60fps[] =
{
	...
	{ 0x3004, 0x00},
	{ 0x3000, 0x01},//pause
	{ 0x3002, 0x01},//continue
	{ 0x3004, 0x04},//restart
	{ 0x3004, 0x00},	
    { 0x3000, 0x01}, // standby
	
	//37Mhz
    //{ 0x300C, 0x5B}, //5B=37Mhz, 0x42=27Mhz
    //{ 0x300D, 0x40},
	//27Mhz
	{ 0x300C, 0x42 },
    { 0x300D, 0x2E },
    ...
    { 0x3018, 0x00},//window mode, 0-default | 3-binning | 4- cropping
    
    { 0x302C, 0x30},//  HTRIMMING Horiz Start in cropping  START
    { 0x302D, 0x00},
    { 0x302E, 0x20},//HNUM Horiz size 0A20 - 2592 cropping size designation
    { 0x302F, 0x0A},//0798 for 1920
    
    { 0x3030, 0x1D},// VMAX default 1194h  , 
    { 0x3031, 0x10},// 
    { 0x3032, 0x00},
    
    //This increases the FPS !!! //14A =50fps //13A =53fps
    { 0x3034, 0x30}, //HMAX default 0226h/0294h is for 25fps,    14A works 38fos
    { 0x3035, 0x01},// 0x014A => 55fps   200 works at 32fps , 1B0 at 38fps, 190 at 41fps with problems!,  

//    { 0x3056, 0xAC}, //Y_OUT_SIZE  effective pixel lines 07ACh
//    { 0x3057, 0x07}, //5B4h = 1440
//    { 0x3072, 0x28},// Vert Crop start 0x28
//    { 0x3073, 0x00},
//    { 0x3076, 0x58},//// AREA3_WIDTH_1 Vert Cropping Size designation * 2 , = Y_OUT_SIZE*2
//    { 0x3077, 0x0F},

    { 0x3050, 0x00},

    //return to defaults not working?!
//    { 0x3074, 0xB0},// AREA3_ST_Addr Upper left cropping postion
//    { 0x3075, 0x00},

    { 0x30C6, 0x00},//Black Offset Addr 12h
    { 0x30CE, 0x00},//UNRD_Line_Max in cropping 
    { 0x30D8, 0x4C},//UNREAD_ED_ADR in cropping
    { 0x30D9, 0x10},
	
	
    
	//{ 0x314C, 0xB0 },//27Mhz 1188Mbps
    { 0x314C, 0x08 },//27Mhz 891Mbps

    //{ 0x314D, 0x02 },//27Mhz 1188Mbps
    { 0x314D, 0x01 },//27Mhz 891Mbps    
    
	//{ 0x315A, 0x02 },//27Mhz 1188Mbps
    { 0x315A, 0x06 },////27Mhz 891Mbps
	
	//{ 0x3168, 0x68 },//37Mhz
	{ 0x3168, 0x8F },//27Mhz
	
    { 0x316A, 0x7E},//both 27Mhz & 37Mhz

    { 0x319D, 0x00},
    { 0x319E, 0x02}, //// SYS_MODE =  891Mbps
	//{ 0x319E, 0x01}, //// SYS_MODE = 1188Mbps
...
const static I2C_ARRAY Sensor_init_table_4lane_5m60fps_1920[] =
{
    /* Stop streaming and enter standby so register writes are accepted safely. */
    {0x3002,0x01}, // XMSTA: master mode operation stop
    {0xffff,0x14},
    {0x3000,0x01}, // STANDBY: enter standby
    {0xffff,0x14},
    {0x3001,0x00}, // REGHOLD: disabled, reflect V-timing registers normally
    {0x3003,0x00},
    {0x3004,0x00}, // RESTART: normal state
    {0x3008,0x00},
    {0x3009,0x00},
    {0x300A,0x00},
    {0x300B,0x00},

    /* INCK-dependent wait timing for 27 MHz input clock, 1188 Mbps/lane table. */
    {0x300C,0x42}, // BCWAIT_TIME
    {0x300D,0x2E}, // CPWAIT_TIME

    /* Window-cropping mode setup and frame geometry. */
    {0x3018,0x04}, // WINMODE: window cropping mode
    {0x301C,0x08},
    {0x3020,0x00},
    {0x3021,0x00},
    {0x3022,0x98},
    ...
    {0x3028,0x38},
    {0x3029,0x0A},
    {0x302C,0x48}, // HTRIMMING_START[11:0]: horizontal crop start
    {0x302D,0x00},
    {0x302E,0x20}, // HNUM[11:0]: horizontal crop width
    {0x302F,0x0A},
    {0x3030,0x60}, // VMAX[19:0]: lines per frame
    {0x3031,0x0F},
    {0x3032,0x00},
    {0x3033,0x00},
    {0x3034,0x3A}, // HMAX[15:0]: clocks per line
    {0x3035,0x01},

    /* Crop-window companion registers from the captured mode dump. */
    {0x3040,0xFF},
    {0x3041,0xFF},
    {0x3042,0x03},
    {0x3043,0x00},
    ...
    /* Output bit depth and exposure timing. */
    {0x3050,0x00}, // ADBIT: 10-bit ADC
    {0x3051,0x02},
    {0x3052,0x00},
    //{0x3058,0xEB}, // SHR0[19:0]: shutter sweep in line units
    //{0x3059,0x08},
    ...
    /* Window-cropping black-level and unread-area controls. */
    {0x30C0,0x00},
    {0x30C1,0x00},
    {0x30C6,0x00}, // BLACK_OFSET_ADR in window cropping mode
    {0x30CC,0x00},
    {0x30CD,0x00},
    {0x30CE,0x00}, // UNRD_LINE_MAX in window cropping mode
    {0x30D8,0x4C}, // UNREAD_ED_ADR in window cropping mode
    {0x30D9,0x10},
    {0x30DA,0x00},
    {0x30E2,0x00},
    {0x30E3,0x00},
    ...
    /* INCK setting for 27 MHz master clock, 1188 Mbps/lane operation. */
    {0x314C,0xB0}, // INCKSEL1[8:0]
    {0x314D,0x00},
    {0x315A,0x02}, // INCKSEL2
    {0x3168,0x8F}, // INCKSEL3
    {0x316A,0x7E}, // INCKSEL4
    {0x319D,0x00}, // MDBIT: 10-bit CSI-2 output
    {0x319E,0x01}, // SYS_MODE: 1188 Mbps/lane table
    {0x31A1,0x00},
    {0x31D7,0x00},
    {0x3288,0x21},
    {0x328A,0x02},
    ...
    /* 10-bit output path: datasheet-required fixed values. */
    {0x3414,0x05}, // datasheet fixed setting
    {0x3416,0x18}, // datasheet fixed setting
    {0x341C,0xFF}, // ADBIT1[8:0]: 10-bit = 0x01FF
    {0x341D,0x01},
    {0x3452,0x19},
    {0x3453,0x00},
    {0x358A,0x77},
    ...
    /* Datasheet fixed analog/digital tuning values for this operating mode family. */
    {0x3648,0x01},
    {0x364A,0x04},
    {0x364C,0x04},
    {0x3678,0x01},
    ...
    /* CSI-2 global timing for 4-lane, 1188 Mbps/lane operation. */
    {0x3A18,0x8F}, // TCLKPOST
    {0x3A1A,0x4F}, // TCLKPREPARE
    {0x3A1C,0x47}, // TCLKTRAIL
    {0x3A1E,0x37}, // TCLKZERO[7:0]
    {0x3A1F,0x01}, // TCLKZERO[9:8]
    ...
    /* Remaining zeroed registers are reserved/disabled in this captured mode. */
    {0x3A42,0x00},
    {0x3A4C,0x00},
    {0x3AE0,0x00},
    {0x3AEC,0x00},
...
const static I2C_ARRAY Sensor_init_table_4lane_90fps[] =
{
    {0x3002,0x01},
    {0xffff,0x14},
    {0x3000,0x01},
...
const static I2C_ARRAY Sensor_init_table_4lane_120fps[] =
{
    {0x3002,0x01},
    {0xffff,0x14},
    {0x3000,0x01},
...
static int pCus_poweron(ms_cus_sensor *handle, u32 idx)
{
    ...
    SENSOR_EMSG("[%s] idx=%u lane_num=%d handle_lane=%u hdr_mode=%u mclk=%d\n",
        __FUNCTION__, idx, lane_num,
        handle->interface_attr.attr_mipi.mipi_lane_num,
        handle->interface_attr.attr_mipi.mipi_hdr_mode,
        handle->mclk);
...
static int pCus_init_mipi4lane_5m60fps_linear(ms_cus_sensor *handle)
{
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_4lane_5m60fps);i++)
    {
        ...
        else
        {
            ...
            printk("\n reg 0x%x, 0x%x",Sensor_init_table_4lane_5m60fps[i].reg, Sensor_init_table_4lane_5m60fps[i].data);
#if 0
            SensorReg_Read(Sensor_init_table_4lane_5m60fps[i].reg, &sen_data );
            if(Sensor_init_table_4lane_5m60fps[i].data != sen_data)
                printk("R/W Differ Reg: 0x%x\n",Sensor_init_table_4lane_5m60fps[i].reg);
            printk("IMX335 reg: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_init_table_4lane_5m30fps[i].reg, Sensor_init_table_4lane_5m30fps[i].data, sen_data);
#endif
        }
    }
...
static int pCus_init_mipi4lane_5m60fps_1920_linear(ms_cus_sensor *handle)
{
    int i,cnt=0;

    SENSOR_DMSG("\n[%s]", __FUNCTION__);

    for(i=0;i< ARRAY_SIZE(Sensor_init_table_4lane_5m60fps_1920);i++)
    {
        if(Sensor_init_table_4lane_5m60fps_1920[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_init_table_4lane_5m60fps_1920[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_init_table_4lane_5m60fps_1920[i].reg,Sensor_init_table_4lane_5m60fps_1920[i].data) != SUCCESS)
            {
                cnt++;
                if(cnt>=10)
                {
...
static int pCus_init_mipi4lane_90fps_linear(ms_cus_sensor *handle)
{
    int i,cnt=0;

    SENSOR_DMSG("\n[%s]", __FUNCTION__);

    for(i=0;i< ARRAY_SIZE(Sensor_init_table_4lane_90fps);i++)
    {
        if(Sensor_init_table_4lane_90fps[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_init_table_4lane_90fps[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_init_table_4lane_90fps[i].reg,Sensor_init_table_4lane_90fps[i].data) != SUCCESS)
            {
                cnt++;
                if(cnt>=10)
                {
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    ...
    u32 num_res = handle->video_res_supported.num_res;
    const char *selected_init = "unset";
    ...
    SENSOR_EMSG("[%s] INIT DRIVER pCus_SetVideoRes->res_idx:[%d]}!\n", __FUNCTION__,res_idx);
    SENSOR_EMSG("[%s]  Sensor res_idx: [%d]  pCus_SetVideoRes !!!!\n", __FUNCTION__,res_idx);
    ...
    switch (res_idx) {
        case 0:
            ...
            if(lane_num == 2){
                ...
                selected_init = "pCus_init_mipi2lane_5m25fps_linear";
            }
            else if (lane_num == 4){
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m25fps_linear;
                selected_init = "pCus_init_mipi4lane_5m25fps_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m25fps_linear;
                selected_init = "pCus_init_mipi4lane_5m25fps_linear";
        ...
        case 1:
            ...
            if(lane_num == 2){
                ...
                selected_init = "pCus_init_mipi4lane_5m30fps_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m30fps_linear;
                selected_init = "pCus_init_mipi4lane_5m30fps_linear";
        ...
        case 2:
            ...
            if(lane_num == 2){
                ...
                selected_init = "pCus_init_mipi4lane_5m60fps_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m60fps_linear;
                selected_init = "pCus_init_mipi4lane_5m60fps_linear";
            ...
            vts_30fps  = 4125;
            Preview_MAX_FPS = 60;
            Preview_line_period = 4040; 

             // Keep internal tVts_reg consistent so GetFPS() is correct immediately
        ...
        case 3:
            ...
            if(lane_num == 2){
                ...
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m60fps_1920_linear;
                selected_init = "pCus_init_mipi4lane_5m60fps_1920_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_5m60fps_1920_linear;
                selected_init = "pCus_init_mipi4lane_5m60fps_1920_linear";
            ...
            vts_30fps  = 3936; // 0x3031:0x3030 = 0x0F60
            Preview_MAX_FPS = 60;
            Preview_line_period = 4253; // HMAX 0x0140 scaled from mode-2 HMAX 0x0130 => 4040 * 320 / 304
        ...
        case 4:
            ...
            if(lane_num == 2){
                ...
                handle->pCus_sensor_init = pCus_init_mipi4lane_90fps_linear;
                selected_init = "pCus_init_mipi4lane_90fps_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_90fps_linear;
                selected_init = "pCus_init_mipi4lane_90fps_linear";
            ...
            vts_30fps  = 3016; // 0x3031:0x3030 = 0x0BC8
            Preview_MAX_FPS = 90;
            Preview_line_period = 3655; // HMAX 0x0113 scaled from mode-2 HMAX 0x0130 => 4040 * 275 / 304
        ...
        case 5:
            ...
            if(lane_num == 2){
                ...
                handle->pCus_sensor_init = pCus_init_mipi4lane_120fps_linear;
                selected_init = "pCus_init_mipi4lane_120fps_linear";
            }
            else{
                handle->pCus_sensor_init = pCus_init_mipi4lane_120fps_linear;
                selected_init = "pCus_init_mipi4lane_120fps_linear";
            ...
            vts_30fps  = 2256; // 0x3031:0x3030 = 0x08D0
            Preview_MAX_FPS = 120;
            Preview_line_period = 3655; // HMAX 0x0113 scaled from mode-2 HMAX 0x0130 => 4040 * 275 / 304
    ...
    handle->bayer_id = imx335_get_linear_bayer_id(handle->video_res_supported.ulcur_res);

    if (invalid_lane_num) {
        SENSOR_EMSG("[%s] unsupported lane_num=%d, fallback init=%s\n",
            __FUNCTION__, lane_num, selected_init);
    }
    SENSOR_EMSG("[%s] res_idx=%u cur_res=%u lane_num=%d handle_lane=%u init=%s fps=%u vts=%u line_period=%u bayer=%u\n",
        __FUNCTION__, res_idx, handle->video_res_supported.ulcur_res,
        lane_num, handle->interface_attr.attr_mipi.mipi_lane_num,
        selected_init, Preview_MAX_FPS, vts_30fps, Preview_line_period,
        handle->bayer_id);
...
int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle)
{
    ...
    handle->bayer_id              = imx335_get_linear_bayer_id(LINEAR_RES_1);
    ...
    SENSOR_EMSG("[%s] init model=%s lane_num=%d hdr_lane_num=%d handle_lane=%u mclk=%d default_res=%u bayer=%u\n",
        __FUNCTION__, handle->model_id, lane_num, hdr_lane_num,
        handle->interface_attr.attr_mipi.mipi_lane_num,
        handle->mclk, handle->video_res_supported.ulcur_res,
        handle->bayer_id);

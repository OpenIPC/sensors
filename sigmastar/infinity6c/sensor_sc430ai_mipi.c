...
#define SENSOR_MAXGAIN 160 // stock driver clamps at 0x28000/1024: 76.47x analog * 2x coarse dgain * fine dgain
...
static struct { // LINEAR
    ...
    { LINEAR_RES_2, { 2688, 1520, 3, 30 }, { 64, 40, 2560, 1440 }, { "2560x1440@30fps" } },
    { LINEAR_RES_3, { 2688, 1520, 3, 60 }, { 0, 0, 2688, 1520 }, { "2688x1520@60fps" } },
};
...
const static I2C_ARRAY gain_reg[] = {
    { 0x3e06, 0x00 }, // dgain coarse: 0 = 1x, 1 = 2x
    { 0x3e07, 0x80 }, // dgain fine, 0x80 = 1.0x, step 1/128
    { 0x3e09, 0x00 }, // again: 00,01,40,48,49,4b,4f,5f
};
...
const static I2C_ARRAY expo_reg[] = {
    ...
    { 0x3e01, 0xcd }, // expo[15:8]
    { 0x3e02, 0xa0 }, // expo[7:4], half-line resolution
};
...
const static I2C_ARRAY vts_reg[] = {
    { 0x320e, 0x06 },
    { 0x320f, 0x72 }, // 1650 lines
};

const static I2C_ARRAY vts_reg_HDR[] = {
    { 0x320e, 0x0c },
    { 0x320f, 0xe4 }, // 3300 lines
};
...
static int pCus_SetFPS(ms_cus_sensor* handle, u32 fps)
{
    ...
    if (params->expo.line > 2 * (params->expo.vts - 5)) {
...
static int pCus_SetAEUSecs(ms_cus_sensor* handle, u32 us)
{
    ...
    sc430ai_params* params = (sc430ai_params*)handle->private_data;
    I2C_ARRAY expo_reg_temp[ARRAY_SIZE(expo_reg)];
    ...
    params->tExpo_reg[0].data = (half_lines >> 12) & 0x0f;
    params->tExpo_reg[1].data = (half_lines >> 4) & 0xff;
    params->tExpo_reg[2].data = (half_lines << 4) & 0xf0;
    params->tVts_reg[0].data = (vts >> 8) & 0x00ff;
    params->tVts_reg[1].data = (vts >> 0) & 0x00ff;
...
// Gain: 1x = 1024. Analog step table and fine/coarse digital gain as in the stock SC430AI driver.
//   again reg 0x3e09:  0x00  0x01  0x40   0x48   0x49   0x4b    0x4f    0x5f
//   again x1024:       1024  2048  2447   4894   9788   19576   39152   78304
// above 78304*2 the coarse dgain (0x3e06) goes to 2x; fine dgain (0x3e07, 1/128) fills the gaps.
static void sc430ai_calc_gain(u32 gain, u16* reg_3e06, u16* reg_3e07, u16* reg_3e09)
{
    u32 again_x1024 = 1024;
    u32 dgain_coarse = 1;
    u16 again_reg = 0x00;
    ...
    *reg_3e09 = again_reg;
    *reg_3e06 = dgain_coarse - 1;
    *reg_3e07 = (u16)(((gain << 7) / again_x1024) / dgain_coarse); // 0x80 = 1.0x
}

static int pCus_SetAEGain(ms_cus_sensor* handle, u32 gain)
{
    sc430ai_params* params = (sc430ai_params*)handle->private_data;
    I2C_ARRAY gain_reg_temp[ARRAY_SIZE(gain_reg)];
    int i;
...
{
    sc430ai_params* params = (sc430ai_params*)handle->private_data;
    I2C_ARRAY gain_reg_temp[ARRAY_SIZE(gain_reg_HDR_SEF)];
    int i;
...
static int sc430ai_GetShutterInfo_HDR_LEF(struct __ms_cus_sensor* handle, CUS_SHUTTER_INFO* info)
{
...
static int sc430ai_GetShutterInfo_HDR_SEF(struct __ms_cus_sensor* handle, CUS_SHUTTER_INFO* info)
{
    sc430ai_params* params = (sc430ai_params*)handle->private_data;
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle)
{
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].width = sc430ai_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].height = sc430ai_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].max_fps = sc430ai_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].min_fps = sc430ai_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].crop_start_x = sc430ai_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].crop_start_y = sc430ai_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].nOutputWidth = sc430ai_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].nOutputHeight = sc430ai_mipi_linear[res].senout.height;
        sprintf(handle->video_res_supported.res[res].strResDesc, sc430ai_mipi_linear[res].senstr.strResDesc);
    }
...
{
    ms_cus_sensor* handle = drv_handle;
    sc430ai_params* params = NULL;
    int res;

    cus_camsensor_init_handle(drv_handle);
    params = (sc430ai_params*)handle->private_data;

    sprintf(handle->model_id, "sc430ai_MIPI_HDR_SEF");
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res + 1;
        handle->video_res_supported.res[res].width = sc430ai_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].height = sc430ai_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].max_fps = sc430ai_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].min_fps = sc430ai_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].crop_start_x = sc430ai_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].crop_start_y = sc430ai_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].nOutputWidth = sc430ai_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].nOutputHeight = sc430ai_mipi_hdr[res].senout.height;
        sprintf(handle->video_res_supported.res[res].strResDesc, sc430ai_mipi_hdr[res].senstr.strResDesc);
    }
    ...
    handle->pCus_sensor_GetShutterInfo = sc430ai_GetShutterInfo_HDR_SEF;
...
int cus_camsensor_init_handle_HDR_LEF(ms_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id, "sc430ai_MIPI_HDR_LEF");
    ...
    handle->pCus_sensor_GetShutterInfo = sc430ai_GetShutterInfo_HDR_LEF;
    handle->pCus_sensor_CustDefineFunction = pCus_sensor_CustDefineFunction;
